    ...
    fetch(`/api/linked-services/${userId}`)
      ...
      .then(data => {
        if (data.success && data.services) {
          const electricityService = data.services.find((s: any) => s.serviceName === "Electricity");
          if (electricityService && electricityService.consumerId) {
            setConsumerId(electricityService.consumerId);
            setMeterForm(prev => ({ ...prev, consumerId: electricityService.consumerId }));
          }
        }
      })
    ...
    try {
      ...
      switch (action) {
        case "outage":
          ...
          description = outageForm.description || outageForm.area;
          break;
        case "new_connection":
          category = "New Connection";
          description = `New connection for ${newConnForm.name} at ${newConnForm.address}`;
          break;
        case "meter_fault":
          category = "Meter Fault";
          description = meterForm.description || meterForm.issue;
          break;
        case "billing_correction":
          category = "Billing Correction";
          description = "Billing correction request";
          break;
      }
      ...
      const res = await fetch("/api/complaints", {
        ...
        body: JSON.stringify({ userId, service, category, description, urgency: "medium" }),
      });
